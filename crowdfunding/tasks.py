...
def task_close_due_campaigns():
    ...
    for campaign in due_campaigns:
        ...
        campaign.save()
        subject, from_email, to = 'Closed Campaign Disbursement Request', settings.ADMIN_EMAIL, campaign.campaign_owner.email
        if campaign.total_funds_received >= campaign.target_amount:
            ...
            disbursement = CampaignDisbursement(campaign=campaign, created_at=timezone.now(), amount=funds_to_disburse, disbursement_type='DISBURSE', disbursement_status='PENDING', recipient=campaign.campaign_owner, recipient_email=campaign.company_email, approval_status='PENDING', approved=False, rejected=False, disbursed=False )
            disbursement.save()
