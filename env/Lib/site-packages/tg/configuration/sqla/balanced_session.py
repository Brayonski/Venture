...
class BalancedSession(Session):
    ...
    def get_bind(self, mapper=None, clause=None):
        ...
        if not engines:
            log.debug('Balancing disabled, using main')
        ...
        if forced_engine:
            ...
            log.debug('Choose engine: main')
            return engines['main']
        else:
            choosen_subordinate = random.choice(list(engines['subordinates'].keys()))
            log.debug('Choose engine: %s', choosen_subordinate)
            return engines['subordinates'][choosen_subordinate]
