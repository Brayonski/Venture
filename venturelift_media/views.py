    ...
    def get_context_categories(self):
        ...
        if current_url == 'media_videos':
            if category == 'startups':
                context['visual'] = AudioVisual.objects.filter(category='Video', sub_category__title__icontains='Enterpreneurship').order_by('date')
            else:
            ...
            if category == 'startups':
                context['audio'] = AudioVisual.objects.filter(category='Podcast',
                                                              sub_category__title__icontains='Enterpreneurship').order_by('date')
            else:
                context['audio'] = AudioVisual.objects.filter(category='Podcast', sub_category__title__icontains=category).order_by('date')
        else:
            if category == 'startups':
                context['media'] = TextMedia.objects.filter(category__title__icontains='Enterpreneurship').order_by('date')
            else:
